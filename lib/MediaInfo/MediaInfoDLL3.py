...
if os.name == "nt" or os.name == "dos" or os.name == "os2" or os.name == "ce":
    """ Modification to the original file
        ...
        Reference
        ---- ----
        1. https://stackoverflow.com/a/27859415
    """
    ...
    assert os.path.exists(configFilePath), \
        f"Required file \"{configFilePath}\" does not exist"
    with open(configFilePath) as f:
        configData = json.load(f)
    mediaInfoDllDirectory = configData.get("MediaInfoDllDirectory", None)
    if mediaInfoDllDirectory is None:
        raise ValueError("Cannot find key \"MediaInfoDllDirectory\" in config file")
    mediaInfoDllPath = os.path.join(mediaInfoDllDirectory, "MediaInfo.dll")
    MediaInfoDLL_Handler = WinDLL(mediaInfoDllPath)
    ## End of modification
    MustUseAnsi = 0
